...
### Best Strategy So Far: {best_strategy_name}
...
**Key Components:**
- Analyzes the factors, transforms, and weights used in the best strategy
- Consider variations: adjust weights, add/remove factors, modify time horizons
- **DO NOT simply copy this spec** - you must create genuinely different strategies
- Use this as inspiration but ensure diversity across your batch

---

### Worst Strategy (AVOID THESE PATTERNS): {worst_strategy_name}
- **Score**: {worst_score:.1f}/100
- **Sharpe Ratio**: {worst_sharpe:.3f}
- **Total Return**: {worst_total_return:.1%}
- **Max Drawdown**: {worst_max_drawdown:.1%}

### Failed Strategy Specification
```json
{worst_strategy_spec}
```
...
- Identify the problematic factor combinations or transforms
- Avoid similar patterns in your new strategies
- This serves as a negative example of what NOT to do

---

### What's Not Working (Best Strategy Issues)
{issues_list}
...
**CRITICAL: If LOFO feedback above identifies harmful columns, DO NOT use those columns in any new strategies.**
...
- **Negation with leading minus**: You may prepend a leading `-` to ANY column in `expr` to invert its direction
  - Examples: `-ret_5d` (reversal), `-rsi_14d` (invert oversold/overbought), `-vol_60d` (prefer low volatility)
...
### 3. Transforms (**EXACTLY 2 required** for each factor)
**Must include these two transforms in this exact order:**
1. `"cs_robust_zscore"` - Cross-sectional normalization (FIRST!)
2. `"winsor:-3,3"` - Outlier handling (SECOND!)

**No other transforms allowed.** DO NOT add a third transform.
...
- **weights**:
  - **If 1 factor**: `[1.0]`
  - **If 2+ factors**: Each weight ∈ [0.20, 0.55], sum EXACTLY to 1.0000 (±0.0001)
...
- [ ] All factors use columns from AVAILABLE COLUMNS (or `-column` to negate any column)
- [ ] Each factor has EXACTLY TWO transforms: `["cs_robust_zscore", "winsor:-3,3"]`
- [ ] Weights: length = # factors; if 1 factor use [1.0]; if 2+ factors, each ∈ [0.20, 0.55]; sum = 1.0
- [ ] Post-processing: exactly `["cs_rank_pct_centered", "clip:-2,2"]`
- [ ] Different from other specs in the batch
- [ ] Different from {best_strategy_name} and {worst_strategy_name}
